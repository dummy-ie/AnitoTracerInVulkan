...
namespace legit
{
    ...
    {
    public:
        GpuProfiler(VkPhysicalDevice physicalDevice, Vulkan::Device logicalDevice, uint32_t maxTimestampsCount) :
            logicalDevice(logicalDevice),
            timestampQuery(physicalDevice, logicalDevice, maxTimestampsCount)
        {
            frameIndex = 0;
        }
        size_t StartTask(std::string taskName, uint32_t taskColor, PipelineStageFlagBits pipelineStageFlags = legit::Colors::turqoise)
        {
        ...
        size_t StartFrame(VkCommandBuffer commandBuffer)
        {
